    ...
    if (get_capture_result == K4A_WAIT_RESULT_SUCCEEDED)
    {
        ...
        if(captureMode == 1 || (captureMode == 2 && isCaptureNow())) {
            ...
            if (colorImage != NULL)
            {
                ...
                cv::imshow( "Display window", colorMat); // Show our image inside it.
                ...
                if(isSaveCapture) {
                    ...
                    char filename[100];
                    if (captureMode == 1) {
                        sprintf(filename, "/ramdisk/capture_%d_%010d_c.png", cameraNo, frameNo);
            ...
            //k4a_image_t depthImage = k4a_capture_get_depth_image(sensor_capture);
            //k4a_image_t depthImage = k4a_capture_get_ir_image(sensor_capture);
            if (depthImage != NULL)
            {
                // you can check the format with this function
                k4a_image_format_t format = k4a_image_get_format(depthImage); // K4A_IMAGE_FORMAT_COLOR_BGRA32 
                ...
                printf(" | image %d  %4dx%4d stride:%5d\n",
                        k4a_image_get_format(depthImage),
                ...
                if(isSaveCapture) {
                    ...
                    char filename[100];
                    if (captureMode == 1) {
                        sprintf(filename, "/ramdisk/capture_%d_%010d_d.png", cameraNo, frameNo);
                    } else if(captureMode == 2) {
                        sprintf(filename, "%s/capture_d.png", prg_dir.c_str());
                        nextOutCaptureJson = true;
                    }
